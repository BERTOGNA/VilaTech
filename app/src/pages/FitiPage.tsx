  ...
  return (
    <div className="bg-[#11110f] text-[#f3f0e8] font-sans min-h-screen scroll-smooth">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 h-[66px] lg:h-[82px] px-6 lg:px-[4vw] flex items-center justify-between bg-[#11110f]/90 backdrop-blur-md border-b border-white/10 transition-all">
        <a href="#inicio" className="flex items-center text-white no-underline w-[175px] lg:w-auto">
          <img src="/images/fiti/marca_FITI.png" alt="FITI" className="max-h-[34px] lg:max-h-[42px] object-contain object-left" />
        </a>
      ...
      <main id="conteudo">
        ...
        <section id="inicio" className="relative min-h-[100svh] grid items-end overflow-hidden bg-[#11110f] text-[#f3f0e8] pt-[82px]">
          ...
          <div className="relative z-10 w-full max-w-[1440px] mx-auto px-6 lg:px-[4vw] pb-[118px]">
            <p className="uppercase tracking-[0.18em] text-[0.73rem] font-extrabold mb-6 fade-up">Itu · São Paulo · proposta para abril de 2027</p>
            <h1 className="font-black uppercase tracking-tighter leading-[0.84] text-[clamp(3.6rem,17vw,9.6rem)] max-w-[10ch] mb-0 fade-up">
              O futuro<br/>encontra <em className="text-[#c8d400] not-italic">lugar</em><br/>em Itu.
            </h1>
            <p className="max-w-[670px] text-[clamp(1.05rem,2vw,1.45rem)] leading-snug mt-8 fade-up">Quatro dias para criatividade, tecnologia e negócios ocuparem a cidade — e transformarem patrimônio em plataforma.</p>
            <div className="flex flex-wrap gap-3 mt-8 fade-up">
              <a href="#visao" className="inline-flex min-h-[52px] items-center justify-center px-5 py-3 uppercase tracking-[0.1em] text-[0.75rem] font-black border border-transparent transition-all hover:-translate-y-0.5 bg-[#c8d400] text-[#11110f] hover:bg-white">Conheça a tese</a>
              <a href="#parcerias" className="inline-flex min-h-[52px] items-center justify-center px-5 py-3 uppercase tracking-[0.1em] text-[0.75rem] font-black border border-white/50 transition-all hover:-translate-y-0.5 text-[#f3f0e8] hover:bg-white hover:text-[#11110f] hover:border-white">Ver proposta para marcas</a>
            </div>
          </div>
        ...
        <section id="visao" className="bg-[#f3f0e8] text-[#11110f] py-[clamp(6rem,11vw,11rem)]">
          <div className="max-w-[1440px] mx-auto px-6 lg:px-[4vw] grid lg:grid-cols-[1fr_4fr] gap-8 lg:gap-[4vw]">
            <p className="uppercase tracking-[0.18em] text-[0.73rem] font-extrabold m-0">02 / A tese</p>
            <div>
        ...
        {/* 03 / Benchmark */}
        <section className="bg-[#11110f] text-[#f3f0e8] py-[clamp(6rem,10vw,10rem)]">
          <div className="max-w-[1440px] mx-auto px-6 lg:px-[4vw]">
            <div className="grid lg:grid-cols-[1.65fr_0.7fr] gap-8 lg:gap-[8vw] items-end mb-[clamp(3rem,7vw,6rem)]">
              <div>
                <p className="uppercase tracking-[0.18em] text-[0.73rem] font-extrabold mb-6">03 / Benchmark traduzido</p>
                <h2 className="font-black uppercase tracking-tighter leading-[0.84] text-[clamp(3rem,14vw,6.8rem)]">Quatro referências.<br/>Um modelo próprio.</h2>
              </div>
              <p className="text-[1.1rem] max-w-[34rem] leading-relaxed">Não é uma réplica em escala menor. É uma síntese desenhada para Itu: curadoria central, produção distribuída e múltiplas portas de entrada.</p>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 border-y border-white/35">
              {[
                { num: '01', title: 'SXSW', desc: 'A cidade como mídia e a convergência como linguagem.' },
                { num: '02', title: 'Rio2C', desc: 'Conteúdo, mercado e festival com jornadas e credenciais distintas.' },
                { num: '03', title: 'Web Summit', desc: 'Startups, investidores e líderes conectados por intenção de negócio.' },
                { num: '04', title: 'SP Tech Week', desc: 'Uma semana descentralizada, produzida com o ecossistema local.' }
              ].map((item, i) => (
                <article key={i} className={`min-h-[270px] lg:min-h-[330px] p-7 flex flex-col ${i % 2 === 0 ? 'border-b md:border-b-0 md:border-r border-white/25' : 'border-b lg:border-b-0 lg:border-r border-white/25'} ${i === 3 ? '!border-r-0' : ''}`}>
                  <span className="text-[#c8d400] font-extrabold text-[0.7rem] tracking-[0.15em]">{item.num}</span>
                  <h3 className="my-auto font-black uppercase tracking-tighter leading-[0.9] text-[clamp(2rem,3vw,3.8rem)]">{item.title}</h3>
                  <p className="min-h-[5em]">{item.desc}</p>
                </article>
              ))}
            </div>
            
            <div className="mt-20 bg-[#f3f0e8] text-[#11110f] p-8 grid lg:grid-cols-[1fr_4fr] gap-4 lg:gap-8 items-center fade-up">
              <small className="uppercase tracking-[0.18em] font-extrabold text-[0.8rem]">A síntese FITI</small>
              <p className="text-[clamp(1.25rem,2.7vw,2.7rem)] leading-tight m-0">
                <strong className="text-[#e83a79] font-normal">cidade-campus</strong> + curadoria de alto nível + mercado ativo + experiências populares
              </p>
            </div>
          </div>
        </section>
        ...
        <section id="formato" className="bg-[#e83a79] text-[#11110f] py-[clamp(6rem,10vw,10rem)]">
          <div className="max-w-[1440px] mx-auto px-6 lg:px-[4vw]">
            <p className="uppercase tracking-[0.18em] text-[0.73rem] font-extrabold mb-6">04 / Arquitetura de experiência</p>
        ...
        {/* 05 / Pilares Editoriais */}
        <section className="bg-[#f3f0e8] text-[#11110f] py-[clamp(6rem,10vw,10rem)]">
          <div className="max-w-[1440px] mx-auto px-6 lg:px-[4vw]">
            <div className="grid lg:grid-cols-[1.65fr_0.7fr] gap-8 lg:gap-[8vw] items-end mb-[clamp(3rem,7vw,6rem)]">
              <div>
                <p className="uppercase tracking-[0.18em] text-[0.73rem] font-extrabold mb-6">05 / Pilares editoriais</p>
                <h2 className="font-black uppercase tracking-tighter leading-[0.84] text-[clamp(3rem,14vw,6.8rem)]">Assuntos que<br/>não vivem isolados.</h2>
              </div>
        ...
        <section id="cidade" className="bg-[#11110f] text-[#f3f0e8] pt-[clamp(6rem,10vw,10rem)] pb-12">
          <div className="max-w-[1440px] mx-auto px-6 lg:px-[4vw] grid lg:grid-cols-[1.65fr_0.7fr] gap-8 lg:gap-[8vw] items-end mb-[clamp(3rem,7vw,6rem)]">
            <div>
              <p className="uppercase tracking-[0.18em] text-[0.73rem] font-extrabold mb-6">06 / Itu como cidade-campus</p>
              <h2 className="font-black uppercase tracking-tighter leading-[0.84] text-[clamp(3rem,14vw,6.8rem)]">A programação<br/>muda de endereço.<br/>A ideia continua.</h2>
            </div>
        ...
        <section className="bg-[#c8d400] text-[#11110f] py-[clamp(5rem,8vw,8rem)]">
          <div className="max-w-[1440px] mx-auto px-6 lg:px-[4vw]">
            <p className="uppercase tracking-[0.18em] text-[0.73rem] font-extrabold mb-6">07 / Ritmo do MVP</p>
        ...
        <section id="curadoria" className="bg-[#f3f0e8] text-[#11110f] py-[clamp(6rem,10vw,10rem)]">
          <div className="max-w-[1440px] mx-auto px-6 lg:px-[4vw]">
            <div className="grid lg:grid-cols-[1.65fr_0.7fr] gap-8 lg:gap-[8vw] items-end mb-[clamp(3rem,7vw,6rem)]">
              <div>
                <p className="uppercase tracking-[0.18em] text-[0.73rem] font-extrabold mb-6">08 / Mapa curatorial</p>
                <h2 className="font-black uppercase tracking-tighter leading-[0.84] text-[clamp(3rem,14vw,6.8rem)]">Vozes que movem<br/>mercados e imaginários.</h2>
              </div>
            ...
            <div className="mt-16 pt-6 border-t-2 border-[#11110f] fade-up">
              <p className="uppercase tracking-[0.12em] text-[0.65rem] font-extrabold mb-4">Rede inicial e acessos facilitados informados pelo Instituto</p>
              <ul className="flex flex-wrap gap-2 text-[0.85rem]">
                {['Kobra', 'Guilherme Kramer', 'Marco Amaro', 'Gustavo Borges', 'Henry Zilbertein', 'João Olivério', 'Ricardo Bellino', 'Ricardo Amorim', 'Rodrigo Helcer', 'João Tabosa'].map((name, i) => (
                  <li key={i} className="border border-[#11110f] px-3 py-2 font-bold">{name}</li>
                ))}
              </ul>
            </div>
          </div>
        ...
        <section id="parcerias" className="bg-[#11110f] text-[#f3f0e8] py-[clamp(6rem,10vw,10rem)]">
          <div className="max-w-[1440px] mx-auto px-6 lg:px-[4vw]">
            <div className="grid lg:grid-cols-[1.65fr_0.7fr] gap-8 lg:gap-[8vw] items-end mb-[clamp(3rem,7vw,6rem)]">
              <div>
                <p className="uppercase tracking-[0.18em] text-[0.73rem] font-extrabold mb-6">09 / Proposta para marcas</p>
                <h2 className="font-black uppercase tracking-tighter leading-[0.84] text-[clamp(3rem,14vw,6.8rem)]">Patrocínio que<br/>não termina no logo.</h2>
              </div>
            ...
            <div className="mt-28 grid grid-cols-1 md:grid-cols-3 border border-white/35 fade-up">
              <div className="bg-[#f3f0e8] text-[#11110f] p-8 border-b md:border-b-0 md:border-r border-white/25">
                <p className="uppercase tracking-[0.18em] text-[0.73rem] font-extrabold mb-6">Arquitetura de recursos</p>
                <h3 className="font-black uppercase tracking-tighter leading-[0.88] text-[clamp(2.5rem,4vw,4.4rem)] mb-8">Dois perímetros.<br/>Uma governança.</h3>
                <p className="opacity-80">Separar escopo, orçamento, entrega e prestação de contas preserva a segurança do projeto e a clareza das contrapartidas.</p>
              </div>
              <article className="p-8 border-b md:border-b-0 md:border-r border-white/25">
                <p className="text-[#c8d400] uppercase tracking-[0.15em] text-[0.68rem] font-extrabold mb-8">Perímetro cultural</p>
                <h4 className="text-[1.5rem] font-bold mb-6">Incentivo + acesso</h4>
                <ul className="list-disc pl-5 opacity-80 space-y-3">
                  <li>Lei Federal de Incentivo à Cultura</li>
                  <li>ProAC ICMS e editais aderentes</li>
                  <li>Programação artística, formativa e gratuita</li>
                  <li>Acessibilidade e democratização de acesso</li>
                </ul>
              </article>
              <article className="p-8">
                <p className="text-[#c8d400] uppercase tracking-[0.15em] text-[0.68rem] font-extrabold mb-8">Perímetro de mercado</p>
                <h4 className="text-[1.5rem] font-bold mb-6">Receita + relacionamento</h4>
                <ul className="list-disc pl-5 opacity-80 space-y-3">
                  <li>Patrocínio direto e propriedades de marca</li>
                  <li>Credenciais profissionais e hospitalidade</li>
                  <li>Exposição, experiências e conteúdo</li>
                  <li>Rodadas, startup market e licenciamento</li>
                </ul>
              </article>
            </div>
            <p className="text-[0.72rem] text-white/60 mt-8 max-w-[1000px] fade-up">Estrutura conceitual sujeita a enquadramento técnico, jurídico, fiscal e orçamentário. Aprovação em mecanismo de incentivo não garante captação. Referências vigentes consultadas em 03/09/2026: IN MinC 29/2026 e ProAC ICMS.</p>
        ...
        {/* 10 / Níveis de Patrocínio */}
        <section className="bg-[#6b4494] text-white py-[clamp(6rem,10vw,10rem)]">
          <div className="max-w-[1440px] mx-auto px-6 lg:px-[4vw]">
            <p className="uppercase tracking-[0.18em] text-[0.73rem] font-extrabold mb-6">10 / Portfólio de parceria</p>
            <h2 className="font-black uppercase tracking-tighter leading-[0.84] text-[clamp(3rem,14vw,8rem)]">Uma entrada para<br/>cada ambição.</h2>
            
            <div className="mt-20 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 border-t border-white/50 fade-up">
              {[
                { num: '01', title: 'Founding Partner', desc: 'Associação estruturante à primeira edição e à plataforma FITI.' },
                { num: '02', title: 'Partner de Pilar', desc: 'Protagonismo editorial em Ser Humano, Economia Criativa ou Techs & IA.' },
                { num: '03', title: 'Partner de Experiência', desc: 'Propriedade de um grande momento, espaço, circuito ou serviço.' },
                { num: '04', title: 'Enabler', desc: 'Tecnologia, mobilidade, conectividade, hospitalidade e infraestrutura.' }
              ].map((tier, i) => (
                <article key={i} className={`min-h-[300px] py-6 lg:p-6 border-b lg:border-b-0 md:border-r border-white/30 flex flex-col ${i === 3 ? 'md:border-r-0 lg:border-r-0' : ''} ${i === 1 ? 'lg:border-r border-white/30' : ''} ${i % 2 !== 0 && i !== 3 ? 'md:border-r-0 lg:border-r' : ''}`}>
                  <span className="text-[#c8d400] text-[0.7rem]">{tier.num}</span>
                  <h3 className="my-auto font-black uppercase leading-[0.9] text-[clamp(1.7rem,3vw,3rem)]">{tier.title}</h3>
                  <p className="opacity-90">{tier.desc}</p>
                </article>
              ))}
            </div>
          </div>
        ...
        <section className="bg-[#f3f0e8] text-[#11110f] py-[clamp(6rem,10vw,10rem)]">
          <div className="max-w-[1440px] mx-auto px-6 lg:px-[4vw] grid lg:grid-cols-[0.75fr_1.6fr] gap-[7vw]">
            <div className="bg-[#11110f] text-[#f3f0e8] p-8 min-h-[360px] flex flex-col items-center justify-center text-center fade-up">
              <p className="uppercase tracking-[0.14em] text-[0.7rem] font-extrabold self-start mb-auto">Realização proponente</p>
              <img src="/images/fiti/instituto-vila-tech-branco.png" alt="Instituto Vila Tech" className="max-w-[280px] w-full my-auto" />
              <span className="text-[0.8rem] text-white/65 mt-auto">Instituto Cultural Vila Tech</span>
            </div>
            <div className="fade-up pt-8 lg:pt-0">
              <p className="uppercase tracking-[0.14em] text-[0.7rem] font-extrabold mb-10">Ecossistema local em articulação</p>
              <div className="flex flex-wrap gap-3">
                {['Vila Tech Hub', 'Fábrica São Luiz', 'FAMA Museu', 'Chou Noodle Bar', 'A Taverna Pub', 'Garagem Pub', 'Empório Santa Ritha', 'Terraço Terras II', 'Do Quintal Pizza', 'A Churrascada Itu', 'Atelier dos Pães', 'Clube do Vinil Café'].map((brand, i) => (
                  <span key={i} className="px-3 py-2 border border-[#11110f] font-extrabold text-[0.85rem]">{brand}</span>
                ))}
              </div>
        ...
        <section className="relative min-h-[92svh] grid items-center overflow-hidden bg-[#11110f] text-[#f3f0e8]">
          ...
          <div className="relative z-10 w-full max-w-[1440px] mx-auto px-6 lg:px-[4vw] py-32 fade-up">
            ...
            <p className="max-w-[690px] text-[1.25rem] my-8">Marcas fundadoras, parceiros de conteúdo, investidores e realizadores: a primeira edição começa na mesa de construção.</p>
            <a href="https://www.vilatechub.com.br/" target="_blank" rel="noreferrer" className="inline-flex min-h-[52px] items-center justify-center px-6 py-3 uppercase tracking-[0.1em] text-[0.75rem] font-black border border-transparent transition-all hover:-translate-y-0.5 bg-[#c8d400] text-[#11110f] hover:bg-white mt-4">
              Abrir conversa com o Instituto ↗
            </a>
          </div>
        </section>
      </main>
